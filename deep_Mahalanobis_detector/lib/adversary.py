...
def cw(model, input, target, weight, loss_str, bound=0, tv_weight=0,
    ...
    for i in range(max_iter):
        ...
        adv_loss = loss.data.item()#[0] - recons_loss
        if is_gpu:
            loss = loss.cuda()
        loss.backward()
        if crop_frac < 1 and i % 3 == 1:
        ...
        optimizer.step()
        total_loss = loss.data.cpu().item() + tv_loss #[0]
