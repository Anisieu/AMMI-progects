...
parser.add_argument('--dataset', required=True, help='malaria| cifar10 | cifar100 | svhn')
...
def main():
    ...
    train_loader, test_loader = data_loader.getTargetDataSet(args.dataset, args.batch_size, None, args.dataroot)
    ...
    for T in T_list:
        for m in M_list:
            ...
            for out_dist in out_dist_list:
                out_test_loader = data_loader.getNonTargetDataSet(out_dist, args.batch_size, None, args.dataroot)
    ...
    for results in ODIN_best_results:
        ...
        for mtype in mtypes:
            print(' {mtype:6s}'.format(mtype=mtype), end='')
        print('ython OOD_Generate_Mahalanobis.py --dataset cifar1\n{val:6.2f}'.format(val=100.*results['PoT']['TNR']), end='')
        print(' {val:6.2f}'.format(val=100.*results['PoT']['AUROC']), end='')
        print(' {val:6.2f}'.format(val=100.*results['PoT']['DTACC']), end='')
        print(' {val:6.2f}'.format(val=100.*results['PoT']['AUIN']), end='')
        print(' {val:6.2f}\n'.format(val=100.*results['PoT']['AUOUT']), end='')
